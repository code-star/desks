...
export async function preDb() {
  ...
  await db.exec("CREATE TABLE IF NOT EXISTS desk (desk_id TEXT PRIMARY KEY, deskstate TEXT)");
    await db.exec( "CREATE TABLE IF NOT EXISTS booking (booking_id TEXT PRIMARY KEY, start_time TEXT, end_time TEXT, date TEXT)");


  interface User{name:string; password:string}
  const allUsers = await db.all<User[]>("SELECT * FROM user WHERE name = (?)", "test" );
  console.log(allUsers);
  if(allUsers.length === 0){
    await db.exec('INSERT INTO user VALUES ("test", "pw")');
    await db.exec('INSERT INTO desk VALUES ("b2.1", "free")');
    await db.exec('INSERT INTO desk VALUES ("b2.2", "free")');
    await db.exec( 'INSERT INTO booking VALUES ("bookingb2.1.1", "13:00", "17:00", "4-11-2021")');
  }
}
